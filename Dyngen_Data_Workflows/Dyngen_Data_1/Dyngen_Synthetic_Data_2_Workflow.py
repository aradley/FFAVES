...
backbone <- bblego(
  ...
  bblego_linear("A", "B", type = "doublerep2", num_modules = 5),
  bblego_branching("B", c("C", "D"), type = "simple",num_modules = 14),
  bblego_end("C", type = "doublerep2", num_modules = 5),
  bblego_linear("D", "E", type = "doublerep2", num_modules = 5),
  bblego_branching("E", c("F", "G"), type = "simple",num_modules = 14),
  bblego_end("F", type = "doublerep2", num_modules = 5),
  bblego_end("G", type = "doublerep2", num_modules = 5)
)
...
config <-
  initialise_model(
    ...
    num_hks = 0,
    verbose = FALSE,
    tf_network_params = tf_network_default(min_tfs_per_module = 10L),
...
write.csv(M1,"Dyngen_Counts.csv")
...
path = "/home/ahr35/Dyngen_Data_Paper/Dyngen_Data_1/"
Dyngen_Counts = pd.read_csv(path + "Dyngen_Counts.csv",header=0,index_col=0)

Dyngen_Embedding = umap.UMAP(n_neighbors=15, min_dist=0.3, n_components=2).fit_transform(np.asarray(Dyngen_Counts))
plt.figure()
plt.scatter(Dyngen_Embedding[:,0],Dyngen_Embedding[:,1])

HK_Counts = pd.read_csv(path + "HK_Genes.csv",header=0,index_col=0)
HK_Counts = HK_Counts.iloc[np.arange(6000)]
...
Num_Noise_Features = 6029
All_Noise_Feature = np.zeros((Dyngen_Counts.shape[0],Num_Noise_Features))
...
for i in np.arange(Num_Noise_Features):
    ...
    All_Noise_Feature[:,i] = Random_Gene
    Gene_IDs[i] = "NG-"+ str(i+1)

Synthetic_Data = np.column_stack((Synthetic_Data,All_Noise_Feature))
...
Dyngen_Embedding = umap.UMAP(n_neighbors=30, min_dist=0.3, n_components=2).fit_transform(np.asarray(Dyngen_Counts))
plt.figure()
plt.scatter(Dyngen_Embedding[:,0],Dyngen_Embedding[:,1])

Dyngen_Embedding = umap.UMAP(n_neighbors=30, min_dist=0.3, n_components=2).fit_transform(np.asarray(Complete_Synthetic_Data))
plt.figure()
plt.scatter(Dyngen_Embedding[:,0],Dyngen_Embedding[:,1])
plt.show()
...
Dyngen_Embedding = umap.UMAP(n_neighbors=30, min_dist=0.3, n_components=2).fit_transform(np.asarray(Dyngen_Counts))
Complete_Synthetic_Data_Embedding = umap.UMAP(n_neighbors=30, min_dist=0.3, n_components=2).fit_transform(np.asarray(Complete_Synthetic_Data))
Dropout_Synthetic_Data_Embedding = umap.UMAP(n_neighbors=30, min_dist=0.3, n_components=2).fit_transform(np.asarray(Dropout_Synthetic_Data))
...
FFAVES_Imputed_Embedding = umap.UMAP(n_neighbors=20, min_dist=0.3, n_components=2).fit_transform(np.asarray(FFAVES_Imputed_Data))
Structured_Genes = Dyngen_Counts.columns
Structured_Genes_FFAVES_Imputed_Embedding = umap.UMAP(n_neighbors=30, min_dist=0.1, n_components=2).fit_transform(np.asarray(FFAVES_Imputed_Data[Structured_Genes]))
...
plt.figure()
plt.scatter(Complete_Embedding[Batch_1,0],Complete_Embedding[Batch_1,1])
plt.scatter(Complete_Embedding[Batch_2,0],Complete_Embedding[Batch_2,1])
plt.figure()
plt.scatter(MAGIC_Embedding[Batch_1,0],MAGIC_Embedding[Batch_1,1])
plt.scatter(MAGIC_Embedding[Batch_2,0],MAGIC_Embedding[Batch_2,1])
plt.figure()
plt.scatter(ALRA_Embedding[Batch_1,0],ALRA_Embedding[Batch_1,1])
plt.scatter(ALRA_Embedding[Batch_2,0],ALRA_Embedding[Batch_2,1])

plt.figure()
plt.scatter(Structured_Complete_Embedding[Batch_1,0],Structured_Complete_Embedding[Batch_1,1])
plt.scatter(Structured_Complete_Embedding[Batch_2,0],Structured_Complete_Embedding[Batch_2,1])
plt.figure()
plt.scatter(Structured_MAGIC_Embedding[Batch_1,0],Structured_MAGIC_Embedding[Batch_1,1])
plt.scatter(Structured_MAGIC_Embedding[Batch_2,0],Structured_MAGIC_Embedding[Batch_2,1])
plt.figure()
plt.scatter(Structured_ALRA_Embedding[Batch_1,0],Structured_ALRA_Embedding[Batch_1,1])
plt.scatter(Structured_ALRA_Embedding[Batch_2,0],Structured_ALRA_Embedding[Batch_2,1])


Structured_Complete_Embedding 
Structured_MAGIC_Embedding
Structured_ALRA_Embedding
...
saver1 <- saver(counts, pred.genes = 1:1000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver2 <- saver(counts, pred.genes = 1001:2000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver3 <- saver(counts, pred.genes = 2001:3000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver4 <- saver(counts, pred.genes = 3001:4000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver5 <- saver(counts, pred.genes = 4001:5000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver6 <- saver(counts, pred.genes = 5001:6000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver7 <- saver(counts, pred.genes = 6001:7000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver8 <- saver(counts, pred.genes = 7001:8000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver9 <- saver(counts, pred.genes = 8001:9000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver10 <- saver(counts, pred.genes = 9001:10000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver11 <- saver(counts, pred.genes = 10001:11000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)
saver12 <- saver(counts, pred.genes = 11001:12000, pred.genes.only = TRUE, 
                do.fast = FALSE, ncores = num_cores)

saver.all <- combine.saver(list(saver1$estimate, saver2$estimate, saver3$estimate, saver4$estimate, saver5$estimate, saver6$estimate, saver7$estimate, saver8$estimate, saver9$estimate, saver10$estimate, saver11$estimate, saver12$estimate))

# Check data is 10000 by 12000
dim(saver.all)

## Un-comment to save results ##
write.csv(saver.all,"Synthetic_Imputation_SAVER.csv")
...
kmeans = KMeans(n_clusters=7, random_state=0).fit(Dyngen_Counts)
...
plt.savefig(path + "Dyngen_1_All_genes.png",dpi=1000)
plt.close()
plt.show()
...
plt.savefig(path + "Dyngen_1_structured_genes.png",dpi=1000)
plt.close()
plt.show()
...
plt.savefig(path + "Dyngen_1_all_genes_sil.png",dpi=1000)
plt.close()
plt.show()

########

Complete_silhouette_vals = silhouette_samples(Complete_Synthetic_Data[Structured_Genes],Cell_Labels)
Drop_Out_silhouette_vals = silhouette_samples(Drop_Out_Synthetic_Data[Structured_Genes],Cell_Labels)
MAGIC_silhouette_vals = silhouette_samples(Synthetic_Imputation_MAGIC[Structured_Genes],Cell_Labels)
SAVER_silhouette_vals = silhouette_samples(Synthetic_Imputation_SAVER[Structured_Genes],Cell_Labels)
FFAVES_silhouette_vals = silhouette_samples(Synthetic_Imputation_FFAVES[Structured_Genes],Cell_Labels)
...
plt.savefig(path + "Dyngen_1_structured_genes_sil.png",dpi=1000)
plt.close()
plt.show()
...
plt.plot(FFAVES_Precisions_1,FFAVES_Recall_1,label="ESFW + FFAVES",linestyle=(0, (5, 10)),linewidth=3)
...
plt.legend(facecolor='white', framealpha=1,fontsize=9.5)

plt.savefig(path + "Dyngen_1_precision_recall.png",dpi=1000)
plt.close()
plt.show()
...
plt.savefig(path + "Dyngen_1_AUC_Curves.png",dpi=1000)
plt.close()
plt.show()
